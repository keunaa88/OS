...
namespace OS_FinalProject
{
    public partial class Form1 : Form
    {
        ...
        public double totalFuelPurchaseLitre = 0, totalFuelPurchaseCost = 0;
        public int totalServices = 0, vehicleId = 1;
        double journeyKilometres = 0;
        double reqiredServiceKilometres = 0;
        ...
        private void btnSaveVehicle_Click(object sender, EventArgs e)
        {
            ...
            Vehicle Vehicle = new Vehicle(vehicle.Id, vehicle.Manufacturer, vehicle.Model, vehicle.RegistrationNo, vehicle.YearOfManufacture, new Journey(), new FuelPurchase(), new PerDayRental(), new PerKmRental(), new Service());
        ...
        private void btnSaveJourney_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Vehicle Vehicle = new Vehicle(vehicle.Id, vehicle.Manufacturer, vehicle.Model, vehicle.RegistrationNo, vehicle.YearOfManufacture, vehicle.Journey, vehicle.FuelPurchase, vehicle.PerDayRental, vehicle.PerKmRental, vehicle.Service);
        ...
        public void CalculateService(Vehicle v)
        {
            ...
            double.TryParse(txtKilometres.Text, out journeyKilometres);
            ...
            if (v.Service.NumberOfService > 0)
            {
        ...
        {
            PerDayRental perDayRental;
            PerKmRental  perKmRental;
            int days = 0;

            if (radiobykilo.Checked)
            {
                perKmRental = new PerKmRental(journeyKilometres);
                perKmRental.calculate_revenue();
                v.PerKmRental = perKmRental;
            }
            else
            {
                int.TryParse(txtDays.Text, out days);
                perDayRental = new PerDayRental(v.PerDayRental.Day + days);
                double rev = perDayRental.calculate_revenue();
        ...
        public void CalculateFuelPurchase(Vehicle v)
        {
            ...
            */
            ...
            double fuelEconomy = 0, fuelPurchaseLitre = 0;

            ////Declaring new model of FuelPurchase 
            FuelPurchase fuelPurchase = new FuelPurchase();
            double.TryParse(txtFuelPurchase.Text, out fuelPurchaseLitre);
        ...
        private void JourneyPrintToScreen(List<Vehicle> listJourney)
        {
            ...
            if (listJourney != null && listJourney.Count > 0)
            {
                foreach (Vehicle vehicle in listJourney)
                {
                    ...
                    //invoiceList.Items.Add("ID: " + c.customerNo + ", Name: " + c.invoiceNo);
                    lvi = new ListViewItem(new string[] { vehicle.Id.ToString(), vehicle.Manufacturer, vehicle.Model, vehicle.YearOfManufacture.ToString(), vehicle.RegistrationNo,
                                                          vehicle.Journey.totalKilometres.ToString(), vehicle.Service.NumberOfService.ToString(), "$"+revenue, vehicle.Journey.kilometresAfterService.ToString(), vehicle.FuelPurchase.FuelEconomy, requireService });
